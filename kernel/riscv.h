...
__attribute__((weak)) uint64 r_stimecmp() {
  uint64 x = 0;
  // If not supported, return 0
  return x;
}

__attribute__((weak)) void w_stimecmp(uint64 x) {
  // If not supported, do nothing
}

// Machine Environment Configuration Register
__attribute__((weak)) uint64 r_menvcfg() {
  uint64 x = 0;
  // If not supported, return 0
  return x;
}

__attribute__((weak)) void w_menvcfg(uint64 x) {
  // If not supported, do nothing
}
